...
| キー | 名前 | 内容 |
|---|---|---|
| 1 | Normal | 同相・r=0.85m・L=8.5m の跳べるシングルループ |
| 2 | Double Loop | 逆相 180°、2ループ探索 |
| 3 | Triple Search | 同相 + 周波数スイープで n=3 を探す |
| 4 | Slick floor | つるつる床(μ=0.02)。r=0.95m・L=8.5m・1.0Hz |
| 5 | Sticky floor | Slick と同じ形状で μ=0.85(比較用) |

Slick は 120 条件のヘッドレスグリッド探索(摩擦 × 回転半径 × 縄長 ×
周波数、jump 指標)の最良セルです。実測値は
開口 1.90m・接地率 12%・mode-1 purity 0.93・jump 1.38(Normal は 1.18)。
Sticky は同じ形状で摩擦だけを上げた比較用で、接地粒子の滑り速度が
5.5 → 2.7 m/s に落ちます。
...
| 4 / 5 | Slick floor(つるつる床)/ Sticky floor(ざらざら床)プリセット |
...
- **Frequency** — 回転周波数 (0–5 Hz)。「左右別周波数」で左右個別にも設定可。
  人が跳べる現実的なケイデンスは ~1.0–1.3 Hz
- **Phase差** — 右手の位相オフセット。180° で逆位相(ダブルダッチ的駆動)
- **Radius** — 手の回転半径。**小さすぎると縄は床を引きずり、
  大きい(0.8〜0.9m)とループが持ち上がって一瞬だけ床をかする**
- **回転方向** — 左右それぞれ正転/逆転
...
- **床との接触** — 片面接触制約の ON/OFF
- **床摩擦 μ** — 接地中の縄が失う接線速度。0 = つるつる(自由に滑る)、
  1 = 接地点がその場に貼り付く。時間刻みに依存しないレート型
  (`keep = exp(-122.6·μ·h)`)なので `Phys dt` を変えても挙動は変わりません。
  既定値は **0.12**(以前は μ=1 相当が固定で入っており、接地部が
  ほぼ貼り付いていました)

摩擦の影響は主に「接地している区間が滑るかどうか」に出ます。既定条件
(f=1.1Hz・r=0.85m・L=8.5m)での接地粒子の平均滑り速度は
μ=0 → 5.4 m/s、μ=0.12 → 4.6 m/s、μ=1 → 2.7 m/s。
モード振幅・開口・接地率はほとんど変わらないため、**摩擦は「モードを決める
パラメータ」ではなく「見た目の引きずり感」を決めるパラメータ**です。
...
- **最適化 Optimizer** — 任意の2パラメータ(周波数/回転半径/縄長/
  ...
  **🧍 跳べる条件を探す** — ワンクリックで人間向け探索を設定:
  周波数を跳べるケイデンス(1.1Hz)に固定し、人が調整できる
  半径×縄長を jump 指標で掃引します
...
- **床**: 片面接触制約(`floorCollision` でON/OFF可)と、接線速度を
  指数減衰させる簡易摩擦(`floorFriction`、時間刻み非依存)。
  実際の大縄と同様に縄は床を叩きます
...
- **跳べる条件(jump 指標での探索)**: 半径0.35mの駆動では縄は床を
  常時引きずり、ループも開口 ~1.0m 止まりで人は入れない
  (数値上のモード支配≠跳べる縄)。
  回転半径 ~0.85m・縄長 ~8.5m・~1.1–1.2Hz で、縄が床を一瞬かする
  (接触 ~10–30%)・開口 ~1.7–1.8m・mode-1 支配の
  **人が実際に跳べるシングルループ**が成立 — 実際の大縄の
  腕の振り・回転速度と整合する条件だった。
  縄長 ~8.0m では張りすぎて床に届かず、~9m+ では引きずり気味 — 
  「一瞬触れる」にはたるみ量の調整が要る。
- **跳べる多重ループはより難しい**: 逆相ダブルループ(r=0.7・L=8.5)
  は mode-2 支配を維持するが、開口 ~1.4m・常時床接触 —
  このモデルの範囲では人が入れる二重ループは見つかっていない。
