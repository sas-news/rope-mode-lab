...
- **最適化 Optimizer** — 任意の2パラメータ(周波数/回転半径/縄長/
  ...
  **粗パス**が全域を低精度モデル(粒子数・反復・刻みを削減)で素早く評価し、
  ...
  「★ 最適セルを適用」で最良条件へジャンプ。
  評価指標は **振幅 / purity / 跳べる度(jump)** から選択:
  jump は「対象モード振幅 × 床接地スコア × 開口スコア」で、
  縄が床を一瞬かすること(常時引きずりは減点)と、
  ループの鉛直開口が設定身長(既定1.7m)を超えることを要求します。
  モードが支配的でも人が入れない状態は高評価になりません。
  Frequency Sweep でも各周波数の開口・接地率を同時記録します
...
- **跳べる条件(jump 指標での探索)**: 半径0.35mの駆動では開口は
  ~1.0m 止まりで人は入れない(数値上のモード支配≠跳べる縄)。
  回転半径 ~0.9m・~1.2Hz で開口 ~1.8m・床接地 ~30%・mode-1 支配の
  **人が実際に跳べるシングルループ**が成立 — 実際の大縄の
  腕の振り・回転速度と整合する条件だった。
